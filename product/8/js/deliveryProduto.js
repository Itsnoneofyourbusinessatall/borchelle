							...
							readJsonFile("../../../delivery/json/loja-" + idUsuario + ".json", function (text) {
								...
								if (continuar) {
									...
									$.ajax({
										...
										success: function (data) {
											$('#modalCarregando').hide();
											// Redirecionar para checkout após adicionar produto
											if (typeof finalizar === 'function') {
												finalizar();
											} else {
												window.location.href = '../../checkout/checkout2.php' + window.location.search;
											}
										},
										error: function () {
											$('#modalCarregando').hide();
											// Fallback: usar localStorage e redirecionar
											if (typeof finalizar === 'function') {
												finalizar();
											} else {
												window.location.href = '../../checkout/checkout2.php' + window.location.search;
